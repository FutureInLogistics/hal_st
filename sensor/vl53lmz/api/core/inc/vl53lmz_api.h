...
/* Raw CNH histogram buffer (used only when CNH plugin is enabled). */
#ifndef VL53LMZ_DISABLE_CNH
#define VL53LMZ_CNH_DATA_SIZE 6160U
#else
#define VL53LMZ_CNH_DATA_SIZE 0U
#endif

/* Motion-indicator "op device" scratch area (used only when MI plugin is enabled). */
#ifndef VL53LMZ_DISABLE_MOTION_INDICATOR
#define VL53LMZ_MI_DATA_SIZE 268U
#else
#define VL53LMZ_MI_DATA_SIZE 0U
#endif

/* Maximum size of (CNH_DATA + MI_OP_DEV). Size-tunable via the VL53LMZ_DISABLE_*
 * compile-time flags above, so applications that don't use CNH or motion indicator
 * do not pay for the scratch buffers they don't need.
 */
#ifndef VL53LMZ_ADDITIONAL_RESULTS_DATA
#define VL53LMZ_ADDITIONAL_RESULTS_DATA (VL53LMZ_CNH_DATA_SIZE + VL53LMZ_MI_DATA_SIZE)
#endif
...
/**
 * @brief This function sets a new ranging frequency in Hz scalled up by 256
