...
#define SI_REVISION_CUT11   0x01
#define SI_REVISION_CUT12   0x02
#define SI_REVISION_L8      0x0C
        ...
        uint8_t             *p_is_alive)
{
    ...
    /* check the data we read back matches the devices this driver supports */
    *p_is_alive = 0;
    if ( (p_dev->device_id==(uint8_t)0xF0) && (p_dev->revision_id==(uint8_t)SI_REVISION_CUT11) )
        *p_is_alive = 1;
    else if ( p_dev->revision_id==(uint8_t)SI_REVISION_CUT12 )
        *p_is_alive = 1;
    else if ( p_dev->revision_id==(uint8_t)SI_REVISION_L8 )
        *p_is_alive = 1;
        ...
        VL53LMZ_Configuration       *p_dev)
{
    ...
    p_dev->module_type = VL53LMZ_MODULE_TYPE_UNKNOWN; /* this will be set to the correct value at the end of this function */
    ...
    /* Enable FW access */
    if ( p_dev->revision_id == (uint8_t)SI_REVISION_L8 ) {
    ...
    {
        status = VL53LMZ_STATUS_FW_CHECKSUM_FAIL;
        goto exit;
    }
    ...
    /* Send appropriate default configuration for the device */
    if ( p_dev->revision_id == (uint8_t)SI_REVISION_L8 ) {
        ...
        uint16_t                    *p_frequency_x256)
{
    ...
    *p_frequency_x256 = (uint16_t)(p_dev->temp_buffer[0x01])<<8 | p_dev->temp_buffer[0x00];
        ...
        uint16_t                    frequency_x256)
{
    ...
    tmp = (uint8_t)(frequency_x256&0xFF);
    status |= vl53lmz_dci_replace_data(p_dev, p_dev->temp_buffer,
                    VL53LMZ_DCI_FREQ_HZ, 4,
                    &tmp, 1, 0x00);

    tmp = (uint8_t)(frequency_x256>>8);
    status |= vl53lmz_dci_replace_data(p_dev, p_dev->temp_buffer,
                    VL53LMZ_DCI_FREQ_HZ, 4,
                    &tmp, 1, 0x01);
        ...
        int16_t                 *p_max_range )
{
    ...
    tmp_16 = (uint16_t)p_dev->temp_buffer[30] | (uint16_t)p_dev->temp_buffer[31]<<8U;
    *p_threshold_pc_x10 = (uint8_t)((tmp_16 * 10U) / 256U);

    tmp_16 = (uint16_t)p_dev->temp_buffer[2] | (uint16_t)p_dev->temp_buffer[3]<<8U;
    *p_max_range =  (int16_t)tmp_16;
        ...
        int16_t                 max_range )
{
   ...
   /* updated the entries in reflectance threshold LUT */
    ...
    scaled_lo = (uint8_t)(scaled_threshold&0xFFU);
    scaled_hi = (uint8_t)(scaled_threshold>>8U);
    p_dev->temp_buffer[30] = scaled_lo;
    p_dev->temp_buffer[31] = scaled_hi;
    p_dev->temp_buffer[32] = scaled_lo;
    p_dev->temp_buffer[33] = scaled_hi;
    p_dev->temp_buffer[34] = scaled_lo;
    p_dev->temp_buffer[35] = scaled_hi;

    p_dev->temp_buffer[2] = (uint8_t)(max_range&0xFFU);
    p_dev->temp_buffer[3] = (uint8_t)(max_range>>8U);
        ...
        uint32_t repeat_count)
{
    uint8_t status = VL53LMZ_STATUS_OK;
    status |= vl53lmz_dci_replace_data(p_dev, p_dev->temp_buffer,
            VL53LMZ_DCI_VHV_CONFIG, 16, (uint8_t*)&repeat_count, 4, 0x4);
    return status;
}
